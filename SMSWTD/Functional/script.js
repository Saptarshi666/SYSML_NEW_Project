...
var disableBreadcrumbs = false;
var currentAnchorHighlighted = '';//This variable contains recently highlighted toc tag
...
function listener(event)
{
	...
	else if ((event.data.substring(0, 11)) === "split_files")
	{
		document.getElementById("main_iframe").src = event.data;
	}
}

function loadFile(url, anchorId, isDisableBreadcrumbs)
{
	...
	if( isDisableBreadcrumbs === undefined) {
		disableBreadcrumbs = false;
	} else {
		disableBreadcrumbs = isDisableBreadcrumbs;
	}

	var iFrameElem = document.getElementById('main_iframe');
	if (iFrameElem === null) {
		window.parent.postMessage(url + anchorId, "*");
	}
	else
	{
...
{
	if (anchorId == null || anchorId == "" || disableBreadcrumbs === true || currentAnchorHighlighted == anchorId)
	{
		return;
	}
	...
	if(alltocs.length == 0 ) {
		//This scrip.js is loaded from iframe and toc is in parent.
		//Hence need to call parent script.js to highlight the current toc.
		var val = anchorId.substring(1);
		window.parent.postMessage("onBacklinkClick|"+val, "*");
		currentAnchorHighlighted = anchorId;
		return;
	}
...
function showBreadcrumbs(contentsArray)
{
	if (contentsArray.length > 0)
	{
		...
		//remove the class attribure from Breadcrumb, since its copied from TOC
		if(tocpath)
		{
			var wrapper = document.createElement('div');
			wrapper.innerHTML= tocpath;

			iterateDOM(wrapper, function(el) {
				if(el.removeAttribute) {
					el.removeAttribute('class');
				}
			});
			tocpath = wrapper.innerHTML;
		}
		//tocpath = tocpath.replace(/(<a[^>]*) class=".*?"/ig, '$1');
...
function getParents(element)
{
	...
	if (parentLevel >=  1 && elemIndex > 1)
	{
		...
		{
			var eleLvl = lis[j].getAttribute("level");
			eleLvl *= 1;
			if (parentLevel > eleLvl)
			{
				...
				currElem.onclick = function(event)
				{
					...
					if (clickX < 15)
					{
						...
						if (this.getAttribute("rel") == "closed")
						{
							...
							this.style.backgroundImage = "url(" + prefix + "minus.gif)"
							expandElements(children);
						}
						else
						{
							this.setAttribute("rel", "closed");
							this.style.backgroundImage = "url(" + prefix + "plus.gif)"
...
function expandElements(list)
{
	...
	var firstLevel = + (firstElem.getAttribute("level") * 1);
	for (var i = 0; i < list.length; i++)
	{
		var elem = list[i];
		var currentLevel = + (elem.getAttribute("level") * 1);
...
function getChildren(element)
{
	...
	for (var j = 0; j < lis.length; j++)
	{
		if (found)
		{
			if ((element.getAttribute("level") * 1) < (lis[j].getAttribute("level") * 1))
			{
				children.push(lis[j]);
			}
			else
